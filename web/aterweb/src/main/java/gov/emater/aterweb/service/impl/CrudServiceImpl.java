...
public abstract class CrudServiceImpl<E extends _ChavePrimaria<CP>, CP extends Serializable, D extends _CrudDao<E, CP>> implements CrudService<E, CP> {
	...
	protected Integer[] calculaNumeroPagina(Integer numeroPagina) {
		if (numeroPagina == null || numeroPagina < 1) {
			return new Integer[] { null, null };
		}
		Integer[] result = { (numeroPagina - 1) * ConstantesService.REGISTROS_POR_PAGINA, ConstantesService.REGISTROS_POR_PAGINA };
		return result;
	}
	...
	public List<CP> getArvoreDescendencia(CP id, String campoId, String campoPai) {
		List<CP> result = getDao().getArvoreDescendencia(id, campoId, campoPai);
		if (result == null) {
