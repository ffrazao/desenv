...
public class ArquivoCarregadoDto implements FiltroDto {
	...
	public ArquivoCarregadoDto(String name, byte[] bytes, String type) throws IOException {
		super();
		setName(name);
		setBytes(bytes);
		setType(type);
	...
	public void setName(String name) {
		...
		if (name != null && name.length() > 0 && name.lastIndexOf(".") >= 0) {
			fileExtension = name.substring(name.lastIndexOf("."), name.length());
		}
	...
	public String toString() {
		return "ArquivoCarregadoDto [name=" + name + ", fileExtension=" + fileExtension + ", size=" + size + ", type=" + type + ", md5=" + md5 + "]";
	}
}