...
var PAGINA = 'usuario-cad';

function cadastroCtrl($scope,$rootScope,$http,$location,toaster,requisicaoService) {
        idsPerfilList = [];
        idsModuloList = [];
        
	requisicaoService.get("dominio?ent=Modulo").success(function(data){
        if(data.executou) {
            $scope.moduloList = data.resultado;
        }
    });
    ...
    $scope.editar = function(){
        if($rootScope.linhasSelecionadas.length === 0) {
            ...
            $scope.emProcessamento(true);
            $http.get(baseUrl + "usuario-cad/restaurar",{params: {id: id}})
            .success(function(data){
                ...
                $.each($rootScope.registro.usuarioModuloList, function(i, data){
                    idsModuloList.push(data.modulo.id);
                });
                
                $.each($rootScope.registro.usuarioPerfilList, function(i, data){
