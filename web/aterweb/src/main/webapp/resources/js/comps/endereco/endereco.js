...
endereco.directive('endereco', function($http, toaster, $rootScope, $q, uiGmapGoogleMapApi) {
	
	return {
		restrict : 'E',
		templateUrl : 'resources/js/comps/endereco/endereco.html',
		scope : {
			dados : "=",
			submitted : '=',
		},
		...
		link : function(scope, element, attrs) {
			...
			scope.atualizaPais = function(lista) {
				$q.all([scope.getDominio("PessoaGrupoPaisVi", null, null, lista)]).then(function(response) {
					...
					if (!scope.dados.pessoaGrupoCidadeVi) {
						scope.dados.pessoaGrupoCidadeVi = {pessoaGrupoMunicipioVi : {pessoaGrupoEstadoVi : {pessoaGrupoPaisVi : {} } } };
					}
...
//				console.log("pesquisa");
				scope.pesquisaGoogle = "";
				if (	!scope.dados ||
						!scope.dados.pessoaGrupoCidadeVi || 
						!scope.dados.pessoaGrupoCidadeVi.pessoaGrupoMunicipioVi || 
						!scope.dados.pessoaGrupoCidadeVi.pessoaGrupoMunicipioVi.pessoaGrupoEstadoVi || 
						!scope.dados.pessoaGrupoCidadeVi.pessoaGrupoMunicipioVi.pessoaGrupoEstadoVi.pessoaGrupoPaisVi || 
						!scope.dados.pessoaGrupoCidadeVi.pessoaGrupoMunicipioVi.pessoaGrupoEstadoVi.pessoaGrupoPaisVi.id ||
						!scope.dados.cep) {
					return;
				}
			...
			scope.buscaCep = function() {
				...
				if (!isUndefOrNull(scope.dados)
					...
					.success(function(data, status, headers, config) {
						if (!data.executou) {
							toaster.pop('error', "CEP {0} não localizado!".format(scope.dados.cep), data.mensagem);
							console.log(data);
						} else {
							...
							if (!scope.dados.propriedadeRural) {
								scope.dados.propriedadeRural = {pessoaGrupoComunidadeVi: {}, pessoaGrupoBaciaHidrograficaVi: {}};
							};
							...
							toaster.pop('info', "CEP localizado", "O CEP {0} foi localizado!".format(scope.dados.cep));
						}
						$rootScope.emProcessamento(false);
					}).error(function(data) {
						toaster.pop('error', "Erro ao acessar o serviço de busca de CEP", data.mensagem);
						console.log(data);
						$rootScope.emProcessamento(false);
					}, true);
				} else {
					toaster.pop('error', "CEP", "Informações incompletas!");
					$rootScope.emProcessamento(false);
				}
			};
			...
			scope.procurarEnderecoPorPessoa = function () {
				...
				.success(function(data, status, headers, config) {
					if (!data.executou) {
						toaster.pop('error', "Nenhum registro localizado!");
						scope.pessoaList = [];
						console.log(data);
					} else {
					...
					toaster.pop('error', "Erro ao consutar o sistema", data.mensagem);
					console.log(data);
					$rootScope.emProcessamento(false);
				}, true);
